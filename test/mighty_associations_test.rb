...
class MightyAssociationsTest < Minitest::Unit::TestCase
  ...
  def test_schema_loaded
    assert_equal([], Company.all.load)
    assert_equal([], Project.all.load)
    assert_equal([], Task.all.load)
    assert_equal([], Item.all.load)
  end
